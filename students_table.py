...
conn = pymysql.connect(
    host="localhost",
    user="root",                   # use your root user or another valid user
    password="Mysql_123", # replace with your password
    database="attendora_db"
)
cursor = conn.cursor()

# ----------------- 2. Drop and create students table -----------------
...
# ----------------- 4. Path to student images -----------------
image_folder = "astudents_images"  # Folder containing all student images

# ----------------- 5. Loop through students and insert -----------------
for student in students_data:
    student_id, roll_no, name, parent_number = student

    # Image filename should match roll_no_name.jpeg (or adjust)
    image_filename = f"{roll_no}_{name}.jpeg"
    ...
    VALUES (%s, %s, %s, %s, %s)
    """
    cursor.execute(insert_query, (student_id, roll_no, name, parent_number, encoding_bytes))
